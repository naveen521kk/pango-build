...
def download_cairo():
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        download(
            f"https://gitlab.freedesktop.org/cairo/cairo/-/archive/{COMMIT_RELEASE_CAIRO}/cairo-{COMMIT_RELEASE_CAIRO}.tar.gz",
            fname,
        )
        with tarfile.open(fname, "r:gz") as tar:
            tar.extractall(tempdir)
        shutil.move(
            str(tempdir / f"cairo-{COMMIT_RELEASE_CAIRO}"), str(build_dir / "cairo")
        )
...
def download_pango():
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        download(
            f"https://ftp.gnome.org/pub/GNOME/sources/pango/{'.'.join(PANGO_VERSION.split('.')[0:2])}/pango-{PANGO_VERSION}.tar.xz",
            fname,
        )
...
def downnload_pkg_config():
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        download(
            f"https://github.com/pkgconf/pkgconf/archive/pkgconf-{PKG_CONFIG_VERSION}.tar.gz",
            fname,
        )
        with tarfile.open(fname, "r") as tar:
            tar.extractall(tempdir)
        shutil.move(
            tempdir / f"pkgconf-pkgconf-{PKG_CONFIG_VERSION}", build_dir / "pkgconf"
        )
...
def download_harfbuzz():
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        download(
            f"https://github.com/harfbuzz/harfbuzz/releases/download/{HARFBUZZ_VERSION}/harfbuzz-{HARFBUZZ_VERSION}.tar.xz",
            fname,
        )
        with tarfile.open(fname, "r") as tar:
            tar.extractall(tempdir)
...
def download_glib():
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        download(
            f"https://ftp.gnome.org/pub/gnome/sources/glib/{'.'.join(GLIB_VERSION.split('.')[0:2])}/glib-{GLIB_VERSION}.tar.xz",
            fname,
        )
        with tarfile.open(fname, "r") as tar:
            tar.extractall(tempdir)
...
call_command(
    command=f"{meson} setup --prefix={(final_loaction/'pkg-config').absolute()} --buildtype=release -Dtests=false pkg_conf_build pkgconf",
    pwd=build_dir,
)
...
environment_varibles[
    "LD_LIBRARY_PATH"
] = f"{(final_loaction /'pkg-config'/'lib'/__lib_dir).absolute()}"
environment_varibles[
    "PATH"
] = f"{(final_loaction/'pkg-config'/'bin').absolute()}{os.pathsep}{environment_varibles['PATH']}"
call_command(
    command=f"ln -sf {(final_loaction/'pkg-config'/'bin'/'pkgconf').absolute()} {(final_loaction/'pkg-config'/'bin'/'pkg-config').absolute()}",
    pwd=build_dir,
)
...
print("Building Glib")
call_command(
    command=f"{meson} setup --default-library=shared --prefix={(final_loaction).absolute()} --buildtype=release glib_builddir glib",
    pwd=build_dir,
)
...
environment_varibles[
    "PATH"
] = f"{(final_loaction/'bin').absolute()}{os.pathsep}{environment_varibles['PATH']}"
environment_varibles[
    "PKG_CONFIG_PATH"
] = f"{(final_loaction/'lib'/__lib_dir/'pkgconfig').absolute()}{os.pathsep}{environment_varibles['PKG_CONFIG_PATH']}"
environment_varibles[
    "LD_LIBRARY_PATH"
] = f"{(final_loaction /'lib'/__lib_dir).absolute()}{os.pathsep}{environment_varibles['LD_LIBRARY_PATH']}"

print("Building Cairo")
# install gperf
# sudo apt-get install gperf
call_command(
    command=f"{meson} setup --default-library=shared --prefix={(final_loaction).absolute()} --buildtype=release -Dfontconfig=enabled -Dfreetype=enabled -Dglib=enabled -Dzlib=enabled -Dtee=enabled cairo_builddir cairo",
    pwd=build_dir,
)
...
call_command(
    command=f"{meson} setup --default-library=shared --prefix={(final_loaction).absolute()} --buildtype=release -Dfontconfig=enabled -Dfreetype=enabled harfbuzz_builddir harfbuzz",
    pwd=build_dir,
)
call_command(command=f"{meson} compile -C harfbuzz_builddir", pwd=build_dir)
call_command(
    command=f"{meson} install --no-rebuild -C harfbuzz_builddir", pwd=build_dir
)

print("Building fribidi")
call_command(
    command=f"{meson} setup --default-library=shared --prefix={(final_loaction).absolute()} --buildtype=release fribidi_builddir fribidi",
    pwd=build_dir,
)
...
call_command(
    command=f"{meson} setup --default-library=shared --prefix={(final_loaction).absolute()} --buildtype=release -Dintrospection=disabled pango_builddir pango",
    pwd=build_dir,
)
